...
async function loadGames() {
  ...
  try {
    ...
    const gameService = window.gameService || (typeof gameService !== 'undefined' ? gameService : null);
...
function renderGamesWithPagination() {
  ...
  // Render games
  gamesContainer.innerHTML = gamesToShow.map(game => createGameCard(game)).join('');
  ...
  console.log(`🎮 Rendered ${gamesToShow.length} of ${allGamesData.length} games (page ${currentPage})`);
}
...
function setupGameCardClickHandlers() {
  ...
  gameCards.forEach(card => {
    card.addEventListener('click', (e) => {
      ...
      const gameData = JSON.parse(card.dataset.gameData.replace(/&apos;/g, "'"));
...
async function filterGamesByCategory(category) {
  ...
  try {
    ...
    console.log(`✅ Found ${result.games.length} games in category: ${category}`);
    ...
    if (gamesContainerElement) {
      gamesContainerElement.innerHTML = result.games.map(game => createGameCard(game)).join('');
      setupGameCardClickHandlers();
    }
        ...
        <div class="w-full">
          ...
          <div class="flex items-center gap-6 text-sm text-gray-600">
            ...
            <div class="flex items-center gap-1">
              ...
              <span>${minPlayers}${maxPlayers !== minPlayers ? `-${maxPlayers}` : ''}</span>
            </div>
            ${rating !== '?' ? 
              `<div class="flex items-center gap-1">
...
function loadGameDetails() {
  ...
  // Update basic info
  document.getElementById('game-title').textContent = game.name || 'Unknown Game';
  document.getElementById('game-subtitle').textContent = game.year ? `Released in ${game.year}` : '';
  ...
  const minPlayers = game.minPlayers || '?';
  const maxPlayers = game.maxPlayers || '?';
  const playerText = minPlayers === maxPlayers ? minPlayers : `${minPlayers}-${maxPlayers}`;
  ...
  if (document.querySelector('.age-value')) {
    document.querySelector('.age-value').textContent = game.age ? `${game.age}+` : '?';
  }
  if (document.querySelector('.rating-value')) {
    document.querySelector('.rating-value').textContent = game.rating ? `${game.rating}/10` : '?';
  }

  // Update designers
  if (game.designers && game.designers.length > 0) {
    document.getElementById('designers-list').innerHTML = 
      game.designers.map(designer => `<p class="text-custom-gray text-2xl">${designer}</p>`).join('');
  } else {
    document.getElementById('designers-list').innerHTML = '<p class="text-custom-gray text-2xl">Unknown</p>';
  }

  // Update publishers
  if (game.publishers && game.publishers.length > 0) {
    document.getElementById('publishers-list').innerHTML = 
      game.publishers.map(publisher => `<p class="text-custom-gray text-2xl">${publisher}</p>`).join('');
  } else {
    document.getElementById('publishers-list').innerHTML = '<p class="text-custom-gray text-2xl">Unknown</p>';
  }
  ...
  if (game.categories && game.categories.length > 0) {
    ...
    document.getElementById('categories-list').innerHTML = '<p class="text-custom-gray text-2xl">None specified</p>';
  }

  // Update detailed description
  document.getElementById('detailed-description').textContent = game.description || 'No detailed description available';
...
function createHomeGameCard(game) {
  const imageUrl = game.image || game.thumbnail || '../../src/asset/images/placeholder.png';
